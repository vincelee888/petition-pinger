...
window.UK.ElectionMap = function () {
  var edgeLength = 5
  var stroke = '#AAAAAA'
  var fill = 'none'
  var origin
  ...
  function chart (svg) {
    ...
    function path (d) {
      var x = origin.x + (d[0] * (edgeLength + cosEdgeLength))
      var y = origin.y - (d[1] * sinEdgeLength)

      var x1 = x + edgeLength
      var y1 = y + sinEdgeLength
      var y2 = y1 + sinEdgeLength

      return closedPath([x, y], [[x1, y], [x1 + cosEdgeLength, y1], [x1, y2], [x, y2], [x - cosEdgeLength, y1]])
    }

    var tiles = svg.selectAll('.constituency').data(window.UK.ElectionMap.CONSTITUENCIES).enter()
      .append('path')
...
window.UK.ElectionMap.CONSTITUENCIES = [
  [0, 0, 'St Ives'],
